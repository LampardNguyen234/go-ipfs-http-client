...
func (c *Client) AddWithMetadata(r io.Reader, metadata map[string]string) (path.Resolved, error){
	unixFs, err := c.GetUnixFs()
	if err != nil {
		return nil, err
	}
	...
	cId, err := c.Unixfs().Add(context.Background(), nodeFile, options.Unixfs.Pin(true))
	if err != nil {
		return nil, err
	}
	...
	return unixFs.AddWithWrapDirectory(context.Background(), metadataNodeFile, options.Unixfs.Pin(true))
}
...
//
// This function also pins the files to the IPFS node.
func (c *Client) AddFileFromPathWithMetadata(filePath string, metadata map[string]string) (path.Resolved, error){
	unixFs, err := c.GetUnixFs()
	if err != nil {
		return nil, err
	}
